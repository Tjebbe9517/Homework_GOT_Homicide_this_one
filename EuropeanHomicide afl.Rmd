...
ggplot(data = Western_Europe, aes(x = Year, y = Homicides_per_100k, color = Entity)) +
    geom_col()+
    labs(title = "Homicides over time", y = "Homicide pr 100k people", x = "Year")
```
...
```{r}
ex_fin_p <- read_csv2("homi/ex_fin.csv")
```
...
ex_fin_p %>% 
  select(Konge_navn, Reign_end, Reign_start, Reign_Length) %>% 
  mutate(Reign_end = as.numeric(Reign_end)) %>% 
  mutate(Reign_start = as.numeric(Reign_start)) %>% 
  mutate(Duration_of_reign = Reign_end-Reign_start) %>% 
  mutate(Midyear = Reign_end - (Reign_end-Reign_start)/2) %>%
  ggplot(aes(x = Midyear, y = Duration_of_reign))+
  geom_smooth()+
  labs(title = "duration of royal rule", x = "years", y = "years in power")
...
ggplot(data = Western_Europe, aes(x = Year, y = Homicides_per_100k, color = Entity)) +
  ...
  theme(legend.position = "bottom")+
    labs(title = "Homicides over time", y = "Homicide pr 100k people", x = "Year", colour = "Country")
...
No, the term "civilized" is a very outdated and problematic way to judge cultures and timeperiods as it strips cultures of their own merits and individuality, comparing them to the west as the "standard norm" of a civilized country. That's why i refuse to answer this question. However in periods of a strong monarch there usually was less crime. However that can't be explored fairly here, as all murders are lumped in under "scandinavia", when we in fact only are interested in those from Denmark.
